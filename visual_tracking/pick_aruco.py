...
if __name__ == "__main__":
    ...
    save_data_dir = "data_states/recorded_arucos_0208/"
    ...
    while not rospy.is_shutdown():
        ...
        if key == ord('q'):
            ...
            if camera_move is None:
                ...
                state = history_states.match(goal_state)
                if state is None:
                    print("No match state found in history for goal_state:", goal_state.ids)
                else:
                    print("Moving to intermediate state:", state.timestamp)
                    # move to the goal state
                    se3_pose = pose_to_SE3(state.robot_pose)
                    robot.goto_pose(se3_pose, wait=False)
                continue
